...
function check(){
	...
	if(document.game.title.value != "") {
		if(document.game.title.value == song){
			score = localStorage.getItem('score');
			score ++;
			localStorage.setItem('score',score); //récupère la valeur du dernier score
			alert("CORRECT TITLE"+document.game.title.value + " "+localStorage.getItem('score'));
		}
		else{
			alert("WRONG TITLE !!!!");
		}
	...
	if(user_singer != "") {
		if(user_singer == singer){
			...
			var score5 = localStorage.getItem('tab_score');
			score5.push(score);
			alert("CORRECT SINGER "+user_singer + " "+localStorage.getItem('score'));
